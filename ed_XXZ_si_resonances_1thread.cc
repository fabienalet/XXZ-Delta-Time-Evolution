...
//          cout << "Sz " << k << " " << sz[k] << " " <<  << endl;
        ...
        if ( (myparameters.measure_participation) || (myparameters.measure_KL) || (measure_sigma_indicator) ) {
        VecCreateSeq(PETSC_COMM_SELF, nconf, &Vec_local1);
        VecCopy(xr, Vec_local1);
        VecGetArray(Vec_local1, &state1);
        }

        int stf=sites_to_follow[ll].size();
        std::vector<Vec> All_sigmaz_as_vec(stf);
        std::vector< *Petscalar > All_sigmaz_as_states(stf);
        ...
        double S1=0.; 
#pragma omp parallel for shared(state1) reduction(+ : S1)
        for (int ri=0;ri<nconf;++ri) {
          double pi=state1[ri];
          if ((pi != 0)) { S1 -= 2.0* pi * pi * log(fabs(pi));}
        }
          double local_S1=0.;
          ...
          partout << "S1 " << global_S1 << " myS1=" << S1 << " " << energies_to_follow[ll] << endl;
        }
        ...
        //if (measure_nearby_eigenstates) { ending=it+}
          for (std::vector<int>::iterator jt=(it+1);jt!=eigenstates_to_follow.end();++jt) {
            ...
            Vec Vec_local2;
            PetscScalar *state2;
            VecCreateSeq(PETSC_COMM_SELF, nconf, &Vec_local2);
        ...
        //  if (it!=jt) {
          ...
          VecCopy(use1, Vec_local2);
          VecGetArray(Vec_local2, &state2);

          if (myparameters.measure_KL) {
          //  if (myparameters.measure_all_KL) {
#pragma omp parallel for shared(state1,state2) reduction(+ : KL1,KL2)
        for (int ri=0;ri<nconf;++ri) {
          double pi=state1[ri];
          double qi=state2[ri];
          if ((pi != 0) && (qi != 0)) {
            KL1 += 2.0 * pi * pi * log(fabs(pi / qi));
            KL2 += 2.0 * qi * qi * log(fabs(qi / pi));
          }
        }
          ...
          KLout << global_KL << " myKL1= " << KL1 << " " << global_KL2 << " myKL1= " << KL2 << " " << energies_to_follow[ll] << " " <<  Er2 << endl;
          }
            }
            // }
          if (measure_sigma_indicator) {
  ...
  for (int ca = 0; ca < basis_pointer->Confs_in_A[nsa].size(); ++ca) {
    double prefactor;
    if (inA) { prefactor = sz_for_basis_state[basis_pointer->Confs_in_A[nsa][ca][d]];}

#pragma omp parallel for shared(state1,state2) reduction(+ : sigma_indicator[si])

    for (int cb = 0; cb < basis_pointer->Confs_in_B[nsa].size(); ++cb) {

    sigma_indicator[si]  += prefactor * state1[i] * state2[i];
