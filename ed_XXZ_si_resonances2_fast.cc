...
int ENV_NUM_THREADS=omp_get_num_threads();
 ...
 //                 MatSetValue(sigmas[k], row_ctr, row_ctr, (PetscScalar)1., ADD_VALUES);
  ...
  for (double &renorm_target : targets) {  // new setup
    ...
    MatCreateVecs(H, NULL, &use1);
     ...
     // std::vector<bool> observables_computed_for_eigenstate(nconv,0);
      for (int i = 0; i < nconv; i++) 
      {
        ...
        PetscScalar *state_i;
        if (myrank == 0) { VecGetArray(Vec_local, &state_i); }
        ...
        for (int j = i+1; j < nconv; j++) 
          ...
          ierr = VecCopy(xr2, Vec_local2);
        }  // only 1 mpi proc
        else {
          VecScatter ctx;
          VecScatterCreateToZero(xr2, &ctx, &Vec_local2);
          VecScatterBegin(ctx, xr2, Vec_local2, INSERT_VALUES, SCATTER_FORWARD);
          VecScatterEnd(ctx, xr2, Vec_local2, INSERT_VALUES, SCATTER_FORWARD);
          VecScatterDestroy(&ctx);
        }
        PetscScalar *state_j;
        if (myrank == 0) {
          VecGetArray(Vec_local2, &state_j);
        }

        //  now do KL only on myrank==0
        double KL1 = 0; double KL2=0.;
        if (myrank==0) {
        for (int p = 0; p < nconf; p++) {
          double ai = PetscAbsScalar(state_i[p]);
          double bi = PetscAbsScalar(state_j[p]);
            if ((ai != 0) && (bi != 0)) {
      KL1 += 2.0 * ai * ai * log(ai / bi);
      KL2 += 2.0 * bi * bi * log(bi / ai);
        }
    if ((KL1>KL_cutoff) && (KL2>KL_cutoff)) { break;}
      }
    ...
    MPI_Bcast(&j_paired[0], jps, MPI_INT, 0, MPI_COMM_WORLD);
      ...
      for (int jp=0;jp<jps;++jp) {
        ...
        if (measure_sigma_indicator) {
              ...
              int nb_large_sigma=0;
              for (int k=0;k<L;++k) { VecDot(use1,sigmas_as_vec[k],&sigma_indicator[k]); 
              if (fabs(sigma_indicator[k])>sigma_cutoff) { nb_large_sigma++; }
              }
              if (myrank==0) {
      ...
      if (jps>0) {
        ...
        std::vector<double> sz(L,0.);
          ...
          // improv en les calculant tous a la fois ...
          if (myrank==0) {
