...
int main(int argc,char **argv)
{
  ...
  // store couplings for computing average energy later
  std::vector<double> couplings (L);
  ...
  /************************** Hamiltonian **********************************/
  {
    ...
    // 
    couplings = myHamiltonian.coupling;
 ...
 /******************************/
  ...
  //sigmas.resize(L);
  std::vector<Vec> sigmax_as_vec;
  sigmax_as_vec.resize(L);
  for (int p=0;p<L;++p) { MatCreateVecs(H, NULL, &sigmax_as_vec[p]);}
  std::vector<Vec> sigmaxsigmax_as_vec;
  // here give the numbers of pairs
  // int nb_pairs=(L*L/2);
  int nb_pairs=2*(L-1)-1;
  sigmaxsigmax_as_vec.resize(nb_pairs);
  for (int p=0;p<nb_pairs;++p) { MatCreateVecs(H, NULL, &sigmaxsigmax_as_vec[p]);}
 ...
 // for (int k=0;k<L;++k) { MatCreateAIJ(PETSC_COMM_WORLD, Iend - Istart, PETSC_DECIDE,nconf,nconf, 0, d_nnz.data(), 0,
    ...
    for (int ca = 0; ca < mybasis.Confs_in_A[nsa].size(); ++ca) {
      ...
      for (int cb = 0; cb < mybasis.Confs_in_B[nsa].size(); ++cb) {
        if ((row_ctr >= Istart) && (row_ctr < Iend)) {
          ...
          for (int k = 0; k < L; ++k) {
                if (config[k]) { VecSetValue(sigmax_as_vec[k],row_ctr,1.,INSERT_VALUES); }
                else { VecSetValue(sigmax_as_vec[k],row_ctr,-1.,INSERT_VALUES); }
          }
        }
        row_ctr++;
      }
  ...
  for (int k = 0; k < L; ++k) {
  VecAssemblyBegin(sigmax_as_vec[k]);
  VecAssemblyEnd(sigmax_as_vec[k]);
  }
  ...
  for (int range=1;range<=(L-1);++range) {
    VecPointwiseMult(sigmaxsigmax_as_vec[running_pair],sigmax_as_vec[k1],sigmax_as_vec[(k1+range)]);
    running_pair++;
  }
  int k2=L-1;
  for (int range=1;range<(L-1);++range) { // don't measure the last one
    VecPointwiseMult(sigmaxsigmax_as_vec[running_pair],sigmax_as_vec[k2],sigmax_as_vec[(k2-range)]);
    running_pair;
  }
  for (int p=0;p<nb_pairs;++p)
  {
  VecAssemblyBegin(sigmaxsigmax_as_vec[p]);
  VecAssemblyEnd(sigmaxsigmax_as_vec[p]);
  }
...
/*********** loop over initial states **********/
  ...
  { // parameters of the initial configuration
    ...
    if (myparameters.measure_local) { myparameters.init_filename_local(locout,i0);	}
    ...
    for (t_index=0;t_index<=myparameters.num_times;++t_index)
    {
      ...
      if ((t_index%each_measurement)==0)
      {
        ...
        for (int k=0;k<L;++k) { 
          ...
          VecPointwiseMult(Psi_t,sigmax_as_vec[k],res);
          VecDotRealPart(Psi_t,res,&sx[k]);  
          }
       // if (myrank==0) { cout << "*** Wathcing dot-real-part= " << sz[0] << " \n";}
        if (myparameters.measure_local) { 
          if (myrank==0) {
          for (int k=0;k<L;++k) { locout << "SX " << i0 << " " << t << " " << k+1 << " " << 0.5*sx[k] << std::endl;}
          }
        }
        ...
        for (int range=1;range<=(L-1);++range) {
          ...
          double C2=0.25*(C-sx[k1]*sx[(k1+range)]);
          if (myrank==0) { corrout << t << " " << k1+1 << " " << k1+range+1 << " " << C << " " << C2 << endl; }
          running_pair++;
          }
        int k2=L-1;
        for (int range=1;range<(L-1);++range) {
